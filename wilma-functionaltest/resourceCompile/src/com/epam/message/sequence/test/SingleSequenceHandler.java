...
public class SingleSequenceHandler implements SequenceHandler {
    public String getExistingSequence(WilmaHttpRequest wilmaHttpRequest, Map<String, WilmaSequence> wilmaSequenceMap, ParameterList parameterList) {
        String result = null;
        String newSequenceKey = this.generateNewSequenceKey(wilmaHttpRequest, parameterList);
        WilmaSequence sequence = (WilmaSequence)wilmaSequenceMap.get(newSequenceKey);
        if (sequence != null) {
            result = newSequenceKey;
        }
        return result;
    }
