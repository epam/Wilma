...
/**
 * Class that creates and starts a new BrowserUp proxy server.
 *
 * @author Tamas Kohegyi
 */
@Component
public class ProxyBrowserUp implements Proxy {

    private final Logger logger = LoggerFactory.getLogger(ProxyBrowserUp.class);
    ...
    @Autowired
    private BrowserUpRequestInterceptor requestFilter;
    @Autowired
    private BrowserUpResponseInterceptor responseFilter;
    @Autowired
    private ProxyConfigurationAccess configurationAccess;

    public static boolean getResponseVolatile() {
        return true; //always volatile
    }

    public ProxyBrowserUp() {
        server = new BrowserUpProxyServer();
    }
    ...
    public static void setResponseVolatile(boolean responseVolatility) {
        //TODO, response is always volatile in BrowserUp
        //BrowserUpProxyServer.setResponseVolatile(responseVolatility);
    }

    public static boolean getShouldKeepSslConnectionAlive() {
        return false;
        //TODO, should keep ssl connection alive feature is not implemented in BrowserUp
        //return BrowserUpProxyServer.getShouldKeepSslConnectionAlive();
    }
    ...
    public static void setShouldKeepSslConnectionAlive(boolean shouldKeepSslConnectionAlive) {
        //TODO, should keep ssl connection alive feature is not implemented in BrowserUp
        //BrowserUpProxyServer.setShouldKeepSslConnectionAlive(shouldKeepSslConnectionAlive);
    }
